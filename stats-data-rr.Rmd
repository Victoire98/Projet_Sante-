...
q<-ggplot(data =  data.stats,aes(x=psu))+
  geom_histogram()+
  xlab("Psu")
...
diag <- get_proportion(data.stats$diag)[1:15,]
...
cont.table.loc <- ((table(data.stats$diag[ind, drop=TRUE],data.stats$race[ind, drop=TRUE])))
res.ca <- CA(cont.table.loc, graph = FALSE)
fviz_ca_biplot(res.ca, repel = TRUE, title = "CA most popular diag and race")
...
ind <- data.stats["output"]=="Knee"
head.sum <- aggregate(data.stats[ind,"output"],by=list(data.stats$trmt_date[ind]), summary)
names(head.sum) <- c('date','freq')
ggplot(data = head.sum, aes(x=date, y=freq)) +
  geom_line()
  #scale_x_date(limit=c(as.Date("2013-01-01"),as.Date("2014-01-01"))) 
